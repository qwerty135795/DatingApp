...
public class UsersController : BaseApiController
{
    ...
    public async Task<ActionResult<PagedList<MemberDTO>>> GetUsers([FromQuery] UserParams param)
    {
        ...
        var users = await _userRepository.GetMembersAsync(param);

        Response.AddPaginationHeaders(
         new PaginationHeaders(users.CurrentPage, users.PageSize
    ...
    [HttpPut]
    public async Task<ActionResult> UpdateUser(MemberUpdateDTO editMemberDTO)
    {
        var user = await _userRepository.GetUserByUsernameAsync(User.GetUsername());
        if (user is null) return NotFound();
        _mapper.Map(editMemberDTO, user);
